...
class SensorFactory(GstRtspServer.RTSPMediaFactory):
    def __init__(self, **properties):
        ...
        self.cap3 = cv2.VideoCapture("/dev/video4")
    ...
    def on_need_data(self, src, length):
        for i, port in enumerate(self.valid_ports):
            cap = cv2.VideoCapture(port)
            ret, frame = cap.read()
            img_list = []
            if ret:
                frame = cv2.putText(frame, i, (200, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                img_list.append(frame)

            frame = cvzone.stackImages(frame, 3, 0.5)

    def on_need_data1(self, src, length):
        ...
        if self.cap1.isOpened():
            ...
            if ret1 or ret2 or ret3:
                ...
                frame = cvzone.stackImages(img, 1, 0.5)
